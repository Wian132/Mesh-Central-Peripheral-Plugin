...
function buildHealthSignalsForPayload(rawHealthSignals, deviceType) {
    ...
    if (String(deviceType || "").toLowerCase() === "admin") {
...
function buildTelemetryPayload(viewState, options) {
    ...
    const healthSignals = buildHealthSignalsForPayload(viewState && viewState.healthSignals, options && options.deviceType);
    if (healthSignals) {
