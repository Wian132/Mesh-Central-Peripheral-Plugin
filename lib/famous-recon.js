...
function getCachedServerId(meshNodeId) {
    return serverIdCache.has(meshNodeId) ? serverIdCache.get(meshNodeId) : null;
}

function setCachedServerId(meshNodeId, serverId) {
    serverIdCache.set(meshNodeId, serverId);
}

function clearServerIdCache() {
    serverIdCache.clear();
}
...
async function sendTelemetryToSupabase(config, payload) {
    ...
    let serverId = getCachedServerId(payload.nodeId);
    if (!serverId) {
        ...
        setCachedServerId(payload.nodeId, serverId);
    }

    const row = buildSupabaseRow(payload, serverId);
    ...
    try {
        ...
        return { ok: true, skipped: false, status: result.status, serverId };
    } catch (error) {
