...
function buildShutdownStatusForPayload(rawShutdownStatus) {
    const shutdownStatus = rawShutdownStatus && typeof rawShutdownStatus === "object" ? rawShutdownStatus : null;
    if (shutdownStatus == null) { return null; }

    const normalized = {
        lastResultStatus:
            shutdownStatus.lastResultStatus == null
                ? null
                : String(shutdownStatus.lastResultStatus || "").trim() || null,
        lastResultAt: toIsoString(shutdownStatus.lastResultAt),
        lastError:
            shutdownStatus.lastError == null
                ? null
                : String(shutdownStatus.lastError || "").trim() || null,
        lastAttemptedSlotKey:
            shutdownStatus.lastAttemptedSlotKey == null
                ? null
                : String(shutdownStatus.lastAttemptedSlotKey || "").trim() || null,
        declinedDate:
            shutdownStatus.declinedDate == null
                ? null
                : String(shutdownStatus.declinedDate || "").trim() || null,
        activeSince: toIsoString(shutdownStatus.activeSince),
        activeUntil: toIsoString(shutdownStatus.activeUntil),
        activeRequestId:
            shutdownStatus.activeRequestId == null
                ? null
                : String(shutdownStatus.activeRequestId || "").trim() || null
    };
...
function buildTelemetryPayload(viewState, options) {
    ...
    const shutdownStatus = buildShutdownStatusForPayload(viewState && viewState.shutdownStatus);
    if (shutdownStatus) {
        payload.shutdownStatus = shutdownStatus;
    }
...
function buildFleetInventoryTelemetryPayload(snapshot, options) {
    const reportedAt = new Date().toISOString();
    return {
        source: "meshcentral-plugin",
        pluginVersion: options && options.pluginVersion ? String(options.pluginVersion) : "",
        reportedAt,
        scanMode: "fleet_inventory",
        nodeId: String(options && options.nodeId || ""),
        deviceId: String(options && options.deviceId || ""),
        deviceType: String(options && options.deviceType || ""),
        fleetDeviceType: "windows_till",
        capturedAt: snapshot && snapshot.capturedAt ? String(snapshot.capturedAt) : reportedAt,
        hostname: snapshot && snapshot.hostname ? String(snapshot.hostname) : String(options && options.deviceId || ""),
        inventoryHash: snapshot && snapshot.inventoryHash ? String(snapshot.inventoryHash) : null,
        inventory: {
            manufacturer: snapshot ? snapshot.manufacturer : null,
            model: snapshot ? snapshot.model : null,
            biosVersion: snapshot ? snapshot.biosVersion : null,
            biosDate: snapshot ? snapshot.biosDate : null,
            windowsEdition: snapshot ? snapshot.windowsEdition : null,
            windowsVersion: snapshot ? snapshot.windowsVersion : null,
            windowsBuild: snapshot ? snapshot.windowsBuild : null,
            windowsArchitecture: snapshot ? snapshot.windowsArchitecture : null,
            installDate: snapshot ? snapshot.installDate : null,
            cpuModel: snapshot ? snapshot.cpuModel : null,
            cpuCores: snapshot ? snapshot.cpuCores : null,
            totalRamGb: snapshot ? snapshot.totalRamGb : null,
            dimmCount: snapshot ? snapshot.dimmCount : null,
            dimmSizesGb: snapshot ? (snapshot.dimmSizesGb || []) : [],
            disks: snapshot ? (snapshot.disks || []) : [],
            primaryNicMac: snapshot ? snapshot.primaryNicMac : null
        },
        warnings: snapshot && Array.isArray(snapshot.warnings) ? snapshot.warnings.slice() : []
    };
}

function buildFleetHealthTelemetryPayload(snapshot, options) {
    const reportedAt = new Date().toISOString();
    return {
        source: "meshcentral-plugin",
        pluginVersion: options && options.pluginVersion ? String(options.pluginVersion) : "",
        reportedAt,
        scanMode: "fleet_health",
        nodeId: String(options && options.nodeId || ""),
        deviceId: String(options && options.deviceId || ""),
        deviceType: String(options && options.deviceType || ""),
        fleetDeviceType: "windows_till",
        ...
        hostname: snapshot && snapshot.hostname ? String(snapshot.hostname) : String(options && options.deviceId || ""),
        health: {
            uptimeHours: snapshot ? snapshot.uptimeHours : null,
            freeSpacePct: snapshot ? snapshot.freeSpacePct : null,
            smartStatus: snapshot ? (snapshot.smartStatus || []) : [],
            bsods30d: snapshot ? (snapshot.bsods30d || []) : [],
            bsods90dCount: snapshot ? snapshot.bsods90dCount : null,
            unexpectedShutdowns30dCount: snapshot ? snapshot.unexpectedShutdowns30dCount : null,
            serviceCrashes7d: snapshot ? (snapshot.serviceCrashes7d || []) : []
        },
        warnings: snapshot && Array.isArray(snapshot.warnings) ? snapshot.warnings.slice() : []
    };
}
