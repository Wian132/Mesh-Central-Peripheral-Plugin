    ...
    function render(state) {
      ...
      app.innerHTML = \`
        <form id="configForm">
          ...
          <div class="panel">
            <h2>Famous Recon Export</h2>
            <div class="sub">Optional: forward normalized scan telemetry into the CentralRecon fleet dashboard using your Famous Recon fleet org deployment key for Admin, POS, or Other devices.</div>
            <div class="grid" style="margin-top:16px;">
              <label><input type="checkbox" name="famousReconEnabled" \${config.integrations.famousRecon.enabled ? 'checked' : ''}> Enable Famous Recon export</label>
            ...
            <div class="grid" style="margin-top:16px;">
              <label>Supabase project URL<input type="text" name="famousReconSupabaseUrl" value="\${esc(config.integrations.famousRecon.supabaseUrl || '')}" placeholder="https://yourproject.supabase.co"></label>
              <label>Supabase anon key<input type="text" name="famousReconSupabaseAnonKey" value="\${esc(config.integrations.famousRecon.supabaseAnonKey || '')}" placeholder="eyJhbGciOi..."></label>
            </div>
            <div class="sub" style="margin-top:8px;">When Supabase URL is set, telemetry inserts directly into <code>server_telemetry</code> via PostgREST. The legacy HTTP endpoint fields below are ignored.</div>
            <div class="grid" style="margin-top:16px;">
              <label>Legacy endpoint URL<input type="text" name="famousReconEndpointUrl" value="\${esc(config.integrations.famousRecon.endpointUrl || '')}" placeholder="https://your-app.example.com/api/fleet/mesh-plugin-telemetry"></label>
              <label>Legacy API key<input type="text" name="famousReconApiKey" value="\${esc(config.integrations.famousRecon.apiKey || '')}" placeholder="fk_..."></label>
      ...
      const payload = {
        ...
        integrations: {
          famousRecon: {
            ...
            supabaseUrl: form.famousReconSupabaseUrl.value.trim(),
            supabaseAnonKey: form.famousReconSupabaseAnonKey.value.trim(),
