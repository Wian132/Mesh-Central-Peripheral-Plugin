      ...
      content.innerHTML = \`
        ...
        <div class="split">
          <div class="card">
            ...
            <div class="meta">
              <span class="pill">Printers +\${(diff.printers.added || []).length}/-\${(diff.printers.removed || []).length}/~\${(diff.printers.changed || []).length}</span>
              <span class="pill">Peripherals +\${(diff.peripherals.added || []).length}/-\${(diff.peripherals.removed || []).length}/~\${(diff.peripherals.changed || []).length}</span>
              <span class="pill">Terminals +\${(diff.paymentTerminalCandidates.added || []).length}/-\${(diff.paymentTerminalCandidates.removed || []).length}</span>
            </div>
          ...
          <div class="card">
            ...
            \${candidates.length ? '<ul class="list">' + candidates.map((candidate) => '<li><strong>' + esc(candidate.name || candidate.instanceId || 'candidate') + '</strong> (' + esc(String(Math.round((candidate.confidence || 0) * 100))) + '%) ' + '<div class="sub">' + esc((candidate.matchedRuleLabels || []).join(', ')) + '</div></li>').join('') + '</ul>' : '<div class="empty">No candidates in the current snapshot.</div>'}
          </div>
        </div>
        ...
        <div class="card table-card">
          ...
          ], peripherals, 'No peripherals detected yet.')}
        </div>\`;
    }
