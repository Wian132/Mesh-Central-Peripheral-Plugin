  ...
  <script>
      ...
      app.innerHTML = \`
        <form id="configForm">
          ...
          <div class="panel">
            ...
            <h3 style="margin-top:20px;">Telemetry Export (Supabase)</h3>
            <div class="sub">When Supabase URL is set, telemetry inserts directly into <code>server_telemetry</code> via PostgREST.</div>
            ...
            <h3 style="margin-top:20px;">Fleet Control (Shutdown Orchestration)</h3>
            <div class="sub">Required for the nightly shutdown waterfall. The plugin derives the <code>/api/fleet/agent-config</code> endpoint from this URL to fetch per-device shutdown decisions.</div>
            <div class="grid" style="margin-top:12px;">
    ...
    async function loadShutdownDebug() {
      ...
      try {
        ...
        healthHtml += '<span style="padding:4px 10px;border-radius:999px;font-size:12px;font-weight:600;background:' + (ch.hasSupabaseUrl ? '#dcfce7;color:#14532d' : '#e5e7eb;color:#6b7280') + ';">Supabase: ' + (ch.hasSupabaseUrl ? 'Set' : 'Not set') + '</span>';
        healthHtml += '</div>';
