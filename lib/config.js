...
const DEFAULT_FLEET_INVENTORY_INTERVAL_HOURS = 168;
const DEFAULT_FLEET_HEALTH_INTERVAL_HOURS = 24;
...
const DEFAULT_CONFIG = Object.freeze({
    ...
    schedule: {
        ...
        fleetInventoryIntervalHours: DEFAULT_FLEET_INVENTORY_INTERVAL_HOURS,
        fleetHealthIntervalHours: DEFAULT_FLEET_HEALTH_INTERVAL_HOURS
    },
    scope: {
        meshIds: [],
        nodeIds: []
    ...
    integrations: {
        famousRecon: {
            ...
            exportOnFleetInventory: true,
            exportOnFleetHealth: true,
...
function hasScopedTargets(scope) {
    return isObject(scope) && (
        ensureArray(scope.meshIds).filter(Boolean).length > 0 ||
        ensureArray(scope.nodeIds).filter(Boolean).length > 0
    );
}

function migrateConfig(input) {
    const candidate = isObject(input) ? deepClone(input) : {};
    const version = Number(candidate.version) || 0;

    // Older installs defaulted the scheduler off. If they already scoped rollout
    // targets, turn scheduled scans on during upgrade so the cadence starts
    // without another manual admin-panel toggle.
    if (version < 2 && hasScopedTargets(candidate.scope) && isObject(candidate.schedule) && candidate.schedule.enabled === false) {
        candidate.schedule.enabled = true;
    }
...
function sanitizeConfig(input) {
    const candidate = migrateConfig(input);
    ...
    if (isObject(candidate.schedule)) {
        ...
        if (Number.isInteger(candidate.schedule.fleetInventoryIntervalHours)) { config.schedule.fleetInventoryIntervalHours = candidate.schedule.fleetInventoryIntervalHours; }
        if (Number.isInteger(candidate.schedule.fleetHealthIntervalHours)) { config.schedule.fleetHealthIntervalHours = candidate.schedule.fleetHealthIntervalHours; }
    }
    ...
    if (isObject(candidate.integrations) && isObject(candidate.integrations.famousRecon)) {
        ...
        if (typeof famousRecon.deviceType === "string") {
            ...
            config.integrations.famousRecon.deviceType = ["server", "pos", "other", "admin"].indexOf(deviceType) >= 0 ? deviceType : "";
        }
        if (typeof famousRecon.forceShutdownAppsClosed === "boolean") { config.integrations.famousRecon.forceShutdownAppsClosed = famousRecon.forceShutdownAppsClosed; }
        ...
        if (typeof famousRecon.exportOnFleetInventory === "boolean") { config.integrations.famousRecon.exportOnFleetInventory = famousRecon.exportOnFleetInventory; }
        if (typeof famousRecon.exportOnFleetHealth === "boolean") { config.integrations.famousRecon.exportOnFleetHealth = famousRecon.exportOnFleetHealth; }
    ...
    if (config.schedule.fleetInventoryIntervalHours < 1 || config.schedule.fleetInventoryIntervalHours > 720) {
        errors.push("schedule.fleetInventoryIntervalHours must be between 1 and 720.");
    }
    if (config.schedule.fleetHealthIntervalHours < 1 || config.schedule.fleetHealthIntervalHours > 168) {
        errors.push("schedule.fleetHealthIntervalHours must be between 1 and 168.");
    }
    ...
    if (config.integrations.famousRecon.enabled) {
        const hasEndpoint = normalizeString(config.integrations.famousRecon.endpointUrl) !== "";
        if (hasEndpoint) {
            if (normalizeString(config.integrations.famousRecon.apiKey) === "") {
                errors.push("integrations.famousRecon.apiKey is required when Famous Recon export is enabled.");
            }
        } else {
            errors.push("integrations.famousRecon.endpointUrl is required when Famous Recon export is enabled.");
        }
    }
...
module.exports = {
    ...
    DEFAULT_FLEET_HEALTH_INTERVAL_HOURS,
    DEFAULT_FLEET_INVENTORY_INTERVAL_HOURS,
