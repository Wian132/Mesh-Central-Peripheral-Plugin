...
function finalizeOfficeEntry(entry) {
    ...
    const licenseDescription = normalizeString(entry.licenseDescription || entry.description);
    const errorCode = normalizeString(entry.errorCode);
    const errorDescription = normalizeString(entry.errorDescription);
    ...
    if (licenseDescription !== "") {
        finalized.officeLicenseDescription = licenseDescription;
    }
    if (errorCode !== "") {
        finalized.officeErrorCode = errorCode;
    }
    if (errorDescription !== "") {
        finalized.officeErrorDescription = errorDescription;
    }
    if (expiresAtRaw !== "") {
...
function parseOsppOfficeEntries(rawOutput) {
    ...
    for (const rawLine of text.split(/\r?\n/)) {
        ...
        const descriptionMatch = line.match(/^\s*LICENSE DESCRIPTION:\s*(.+?)\s*$/i);
        if (descriptionMatch) {
            if (!current) { current = {}; }
            current.licenseDescription = descriptionMatch[1];
            continue;
        }

        const errorCodeMatch = line.match(/^\s*ERROR CODE:\s*(.+?)\s*$/i);
        if (errorCodeMatch) {
            if (!current) { current = {}; }
            current.errorCode = errorCodeMatch[1];
            continue;
        }

        const errorDescriptionMatch = line.match(/^\s*ERROR DESCRIPTION:\s*(.+?)\s*$/i);
        if (errorDescriptionMatch) {
            if (!current) { current = {}; }
            current.errorDescription = errorDescriptionMatch[1];
        }
    }
...
function parseVnextOfficeEntries(rawOutput) {
    ...
    for (const block of objectMatches) {
        try {
            ...
            const finalized = finalizeOfficeEntry({
                ...
                licenseDescription: parsed.Description || parsed.LicenseDescription,
    ...
    for (const rawLine of text.split(/\r?\n/)) {
        ...
        match = line.match(/^(?:Description|LicenseDescription)\s*:\s*(.+?)\s*$/i);
        if (match) {
            current = current || {};
            current.licenseDescription = match[1];
            continue;
        }

        match = line.match(/^(?:ErrorCode)\s*:\s*(.+?)\s*$/i);
        if (match) {
            current = current || {};
            current.errorCode = match[1];
            continue;
        }

        match = line.match(/^(?:ErrorDescription)\s*:\s*(.+?)\s*$/i);
        if (match) {
            current = current || {};
            current.errorDescription = match[1];
            continue;
        }
...
function normalizeHealthSignals(rawHealthSignals) {
    ...
    if (hasOwn(healthSignals, "office")) {
        ...
        if (hasOwn(office, "officeLicenseDescription")) {
            normalized.officeLicenseDescription = office.officeLicenseDescription;
        }
        if (hasOwn(office, "officeErrorCode")) {
            normalized.officeErrorCode = office.officeErrorCode;
        }
        if (hasOwn(office, "officeErrorDescription")) {
            normalized.officeErrorDescription = office.officeErrorDescription;
        }
    }
...
function mergeHealthSignals(statusSnapshot, fullSnapshot) {
    ...
    if (fullHealth && hasOwn(fullHealth, "officeLicenseDescription")) {
        merged.officeLicenseDescription = fullHealth.officeLicenseDescription;
    }
    if (fullHealth && hasOwn(fullHealth, "officeErrorCode")) {
        merged.officeErrorCode = fullHealth.officeErrorCode;
    }
    if (fullHealth && hasOwn(fullHealth, "officeErrorDescription")) {
        merged.officeErrorDescription = fullHealth.officeErrorDescription;
    }
...
function normalizeFleetInventoryPayload(rawPayload) {
    const payload = parseAgentPayload(rawPayload);

    const disks = ensureArray(payload.disks).map((d) => ({
        model: nonEmptyString(d && d.model),
        media_type: nonEmptyString(d && d.media_type),
        size_gb: toNumberOrNull(d && d.size_gb),
        serial: nonEmptyString(d && d.serial)
    }));

    const dimmSizesGb = ensureArray(payload.dimmSizesGb)
        .map((v) => toNumberOrNull(v))
        .filter((v) => v != null);

    const snapshot = {
        capturedAt: toIsoOrNull(payload.capturedAt) || new Date().toISOString(),
        hostname: nonEmptyString(payload.hostname),
        manufacturer: nonEmptyString(payload.manufacturer),
        model: nonEmptyString(payload.model),
        biosVersion: nonEmptyString(payload.biosVersion),
        biosDate: toIsoOrNull(payload.biosDate),
        windowsEdition: nonEmptyString(payload.windowsEdition),
        windowsVersion: nonEmptyString(payload.windowsVersion),
        windowsBuild: nonEmptyString(payload.windowsBuild),
        windowsArchitecture: nonEmptyString(payload.windowsArchitecture),
        installDate: toIsoOrNull(payload.installDate),
        cpuModel: nonEmptyString(payload.cpuModel),
        cpuCores: toIntOrNull(payload.cpuCores),
        totalRamGb: toNumberOrNull(payload.totalRamGb),
        dimmCount: toIntOrNull(payload.dimmCount),
        dimmSizesGb,
        disks,
        primaryNicMac: nonEmptyString(payload.primaryNicMac),
        warnings: ensureArray(payload.warnings)
    };

    // Hash covers only fields that should trigger an upsert; capturedAt + warnings are
    // excluded so a no-op collection does not ping the hub every week.
    const hashBody = {
        ...
        windowsArchitecture: snapshot.windowsArchitecture,
        installDate: snapshot.installDate,
        cpuModel: snapshot.cpuModel,
        cpuCores: snapshot.cpuCores,
        totalRamGb: snapshot.totalRamGb,
        dimmCount: snapshot.dimmCount,
        dimmSizesGb: snapshot.dimmSizesGb.slice().sort(),
        disks: snapshot.disks
            .map((d) => ({ model: d.model, media_type: d.media_type, size_gb: d.size_gb, serial: d.serial }))
            .sort((a, b) => String(a.serial || a.model || "").localeCompare(String(b.serial || b.model || ""))),
        primaryNicMac: snapshot.primaryNicMac
    };
    snapshot.inventoryHash = createHashHex(JSON.stringify(hashBody));

    return snapshot;
}

function normalizeFleetHealthPayload(rawPayload) {
    const payload = parseAgentPayload(rawPayload);

    const smartStatus = ensureArray(payload.smartStatus).map((d) => ({
        friendly_name: nonEmptyString(d && d.friendly_name),
        health_status: nonEmptyString(d && d.health_status),
        operational_status: ensureArray(d && d.operational_status).map((s) => String(s || "")).filter(Boolean),
        media_type: nonEmptyString(d && d.media_type),
        predict_failure: typeof (d && d.predict_failure) === "boolean" ? d.predict_failure : null,
        reason_code: toIntOrNull(d && d.reason_code)
    }));

    const bsods30d = ensureArray(payload.bsods30d).map((e) => ({
        time_created: toIsoOrNull(e && e.time_created),
        bugcheck_code: nonEmptyString(e && e.bugcheck_code),
        bugcheck_text: nonEmptyString(e && e.bugcheck_text)
    }));

    const serviceCrashes7d = ensureArray(payload.serviceCrashes7d)
        .map((g) => ({
            process: nonEmptyString(g && g.process),
            count: toIntOrNull(g && g.count)
        }))
        .filter((g) => g.process && g.count != null)
    ...
    return {
        capturedAt: toIsoOrNull(payload.capturedAt) || new Date().toISOString(),
        hostname: nonEmptyString(payload.hostname),
        uptimeHours: toNumberOrNull(payload.uptimeHours),
        freeSpacePct: toNumberOrNull(payload.freeSpacePct),
        smartStatus,
        bsods30d,
        bsods90dCount: toIntOrNull(payload.bsods90dCount),
        unexpectedShutdowns30dCount: toIntOrNull(payload.unexpectedShutdowns30dCount),
        serviceCrashes7d,
        warnings: ensureArray(payload.warnings)
    };
}
